...
app.post('/api/firewall', authMiddleware, requireWrite, async (req, res) => {
  ...
  const result = await dbRun('INSERT INTO firewall_requests (req_date,corp,dept,requester,status,title,worker,doc,memo) VALUES (?,?,?,?,?,?,?,?,?)',
...
app.put('/api/firewall/:id', authMiddleware, requireWrite, async (req, res) => {
  const { req_date, corp, dept, requester, status, title, worker, doc, memo } = req.body;
  await dbRun("UPDATE firewall_requests SET req_date=?,corp=?,dept=?,requester=?,status=?,title=?,worker=?,doc=?,memo=?,updated_at=NOW() WHERE id=?",
